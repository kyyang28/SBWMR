...
//	for (int motorIndex = 0; motorIndex < MAX_SUPPORTED_DC_BRUSHED_MOTORS_FOR_SBWMR; motorIndex++) {
		...
		IO_t PWMA = IOGetByTag(dcBrushedMotorConfig->ioTagsPWM[0]);
		IO_t PWMB = IOGetByTag(dcBrushedMotorConfig->ioTagsPWM[1]);
		const timerHardware_t *timerHardware_A = timerGetByTag(dcBrushedMotorConfig->ioTagsPWM[0], TIM_USE_MOTOR);
		const timerHardware_t *timerHardware_B = timerGetByTag(dcBrushedMotorConfig->ioTagsPWM[1], TIM_USE_MOTOR);
		
//		printf("timer A channel: %u\r\n", timerHardware_A->channel);		// 0x0
//		printf("timer B channel: %u\r\n", timerHardware_B->channel);		// 0x4
//		printf("IO_GPIO(PWMA): 0x%x\r\n", (uint32_t)IO_GPIO(PWMA));
//		printf("IO_GPIO(PWMB): 0x%x\r\n", (uint32_t)IO_GPIO(PWMB));
//		printf("IO_Pin(PWMA): %u\r\n", IO_Pin(PWMA));
//		printf("IO_Pin(PWMB): %u\r\n", IO_Pin(PWMB));
	
		/* IO init */
		IOInit(PWMA, OWNER_MOTOR, 4);
		IOInit(PWMB, OWNER_MOTOR, 5);
		IOConfigGPIOAF(PWMA, IOCFG_AF_PP, timerHardware_A->alternateFunction);
		IOConfigGPIOAF(PWMB, IOCFG_AF_PP, timerHardware_B->alternateFunction);
//		IOConfigGPIOAF(io, IOCFG_AF_PP_PD, timerHardware->alternateFunction);
		
		/* Time base structure configuration */
		configTimeBase4Encoder(timerHardware_A->tim, 7200, 0);	// ARR = 7200 - 1 = 7199, PSC = 0
		
		/* Output compare PWM generator configuration */
		pwmOCConfig(timerHardware_A->tim, timerHardware_A->channel, 0, timerHardware_A->output);
		pwmOCConfig(timerHardware_B->tim, timerHardware_B->channel, 0, timerHardware_B->output);
		
		/* For advanced TIMER, TIM1 and TIM8 to output PWM signals */
		TIM_CtrlPWMOutputs(timerHardware_A->tim, ENABLE);
		
		TIM_Cmd(timerHardware_A->tim, ENABLE);				// Enable TIMER		

		dcBrushedMotors[0].ccr = timerChCCR(timerHardware_A);	// Config the channel address for duty cycle
		dcBrushedMotors[0].period = 7200;						// Config the PWM period
		dcBrushedMotors[0].tim = timerHardware_A->tim;			// Config the Timer number
		
		*dcBrushedMotors[0].ccr = 0;							// No duty cycle

		dcBrushedMotors[1].ccr = timerChCCR(timerHardware_B);	// Config the channel address for duty cycle
		dcBrushedMotors[1].period = 7200;						// Config the PWM period
		dcBrushedMotors[1].tim = timerHardware_B->tim;			// Config the Timer number
		
		*dcBrushedMotors[1].ccr = 0;							// No duty cycle
//	}
}
